...
$purifier = new HTMLPurifier($config);
  ...
  // 创建目录
  $dirName = __DIR__ ."/../images/xx_{$id}/case_picture/{$postData['title']}/";
  if(!is_dir($dirName))
  ...
  // 判断图片类型
    if($cover['type'] == "image/jpeg"){
      ...
      if($gd->resizeImage( $im, 500, 800, $dirName . $filename )){
        $postData['picture'] = $dbh::url . "images/xx_{$id}/case_picture/{$postData['title']}/".$filename;
  ...
  $sql = "INSERT INTO adv_company_case
         (title, 
          process_title, 
          process, 
          cover, 
  ...
  $params = array(':title' => $postData['title'], 
                  ':process_title' => $postData['process_title'],
                  ':process' => $postData['process'],
                  ':cover' => $postData['picture'],
                  ':content' => $content,
                  ':id' => $id);
