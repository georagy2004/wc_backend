...
$purifier = new HTMLPurifier($config);
  ...
  // 创建目录
  $dirName = __DIR__ ."/../images/xx_{$id}/project_picture/{$postData['title']}/";
  if(!is_dir($dirName))
  ...
  // 判断图片类型
    if($cover['type'] == "image/jpeg"){
      ...
      if($gd->resizeImage( $im, 500, 800, $dirName . $filename )){
        $postData['picture'] = $dbh::url . "images/xx_{$id}/project_picture/{$postData['title']}/".$filename;
        $postData['picture'] = urlencode($postData['picture']);
      }else{
        exit(ErrorCode::E_UPLOAD_PICTURE);
      }
    }else if($cover['type'] == "image/png"){
      $im = imagecreatefrompng($cover['tmp_name']);
      if($gd->resizeImage( $im, 500, 800, $dirName . $filename )){
        $postData['picture'] = $dbh::url . "images/xx_{$id}/project_picture/{$postData['title']}/".$filename;
        $postData['picture'] = urlencode($postData['picture']);
      }else{
  ...
  // 对富文本进行XSS过滤
  $content = $purifier->purify($postData['content']);
  
  //  添加到数据库
  $sql = "INSERT INTO adv_company_project
         (title, cover, content, adv_company_user_id) 
         VALUES
         (:title, :cover, :content, :id)";
  $params = array(':title' => $postData['title'], ':cover' => $postData['picture'], ':content' => $content, ':id' => $id);
